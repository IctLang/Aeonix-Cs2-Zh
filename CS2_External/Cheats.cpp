...
void Cheats::Menu()
{
	...
	ImGui::Begin(u8"Aeonix插件 zh泽", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
	{
		...
		if (ImGui::BeginChild(2, ImVec2(485, 600), true)) {
			...
			if (tabb == 0) {
				//esp
				ImGui::SeparatorText(u8"绘制");

				ImGui::Checkbox(u8"方框", &MenuConfig::ShowBoxESP);
				...
				ImGui::Combo(u8"血条类型", &MenuConfig::HealthBarType, u8"垂直\0顶部\0底部");



				ImGui::Checkbox(u8"骨骼", &MenuConfig::ShowBoneESP);
				if (ImGui::IsItemClicked(1))
				{
				...
				ImGui::Checkbox(u8"视线", &MenuConfig::ShowEyeRay);
				ImGui::SameLine();
				ImGui::ColorEdit4("##EyeRay", reinterpret_cast<float*>(&MenuConfig::EyeRayColor), ImGuiColorEditFlags_NoInputs);


				ImGui::Checkbox(u8"距离", &MenuConfig::ShowDistance);
				ImGui::Checkbox(u8"血量", &MenuConfig::HealthText);

				ImGui::Checkbox(u8"武器名", &MenuConfig::ShowWeaponESP);
				ImGui::Checkbox(u8"玩家名", &MenuConfig::ShowPlayerName);

				ImGui::Checkbox(u8"敌人连线", &MenuConfig::ShowLineToEnemy);
				ImGui::SameLine();
				ImGui::ColorEdit4("##SnapLine", reinterpret_cast<float*>(&MenuConfig::LineToEnemyColor), ImGuiColorEditFlags_NoInputs);
				ImGui::SameLine();
				ImGui::SetNextItemWidth(110);
				ImGui::Combo(u8"连线位置", &MenuConfig::SnapLinePos, u8"顶部\0中心\0底部");

				ImGui::Checkbox(u8"绘制预览", &MenuConfig::ESPPreview);
				ImGui::Checkbox(u8"功能列表", &MenuConfig::CheatList);



				ImGui::SeparatorText(u8"辅助线");
				ImGui::Checkbox(u8"准心", &MenuConfig::ShowCrossHair);
				ImGui::SameLine();
				...
				{
					ImGui::TextUnformatted(u8"设置");
					ImGui::Checkbox(u8"中心点", &CrosshairConfig::drawDot);
					ImGui::SliderInt(u8"水平长度", &CrosshairConfig::HorizontalLength, 0, 75, "%d");
					ImGui::SliderInt(u8"垂直高度", &CrosshairConfig::VerticalLength, 0, 75, "%d");
					ImGui::SliderInt(u8"间距", &CrosshairConfig::Gap, 0, 35, "%d");
					ImGui::Checkbox(u8"描边", &CrosshairConfig::drawOutLine);
					ImGui::Checkbox(u8"T形准心", &CrosshairConfig::tStyle);
					ImGui::EndPopup();
				}

				ImGui::Checkbox(u8"爆头线", &MenuConfig::ShowHeadShootLine);
				ImGui::SameLine();
				ImGui::ColorEdit4("##HeadShootLineColor", reinterpret_cast<float*>(&MenuConfig::HeadShootLineColor), ImGuiColorEditFlags_NoInputs);


				ImGui::Checkbox(u8"视场线", &MenuConfig::ShowFovLine);
				ImGui::SameLine();
				...
				Gui.SliderScalarEx1(u8"视场线尺寸", ImGuiDataType_Float, &MenuConfig::FovLineSize, &FovLineSizeMin, &FovLineSizeMax, "%.1f", ImGuiSliderFlags_None);
				...
				if (MenuConfig::ESPPreview) {
					...
					const auto& window = ImGui::FindWindowByName(u8"Aeonix插件 zh泽");
					ImGui::SetNextWindowPos(ImVec2(window->Pos.x + window->Size.x + ImGui::GetStyle().WindowPadding.x, window->Pos.y));
					ImGui::End();
					ImGui::Begin(u8"绘制预览", &MenuConfig::ESPPreview, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
					...
					if (MenuConfig::ShowPlayerName) {
						if (MenuConfig::HealthBarType == 0 || MenuConfig::HealthBarType == 2) {
							...
							ImGui::GetWindowDrawList()->AddText(textPos, IM_COL32(255, 255, 255, 255), u8"玩家名");
						}
						if (MenuConfig::HealthBarType == 1) {
					...
					if (MenuConfig::ShowWeaponESP) {
						if (MenuConfig::HealthBarType == 2) {
							...
							ImGui::GetWindowDrawList()->AddText(textPos, IM_COL32(255, 255, 255, 255), u8"武器名");
						}
				...
				if (ImGui::Combo(u8"瞄准部位", &MenuConfig::AimPosition, u8"头部\0脖子\0颈椎"))
				{
				...
				Gui.SliderScalarEx1(u8"起始子弹", ImGuiDataType_U32, &AimControl::RCSBullet, &BulletMin, &BulletMax, "%1.f", ImGuiSliderFlags_None);
				Gui.SliderScalarEx1(u8"子弹后座", ImGuiDataType_Float, &AimControl::RCSScale.x, &RecoilMin, &RecoilMax, "%.1f", ImGuiSliderFlags_None);
				Gui.SliderScalarEx1(u8"子弹散布", ImGuiDataType_Float, &AimControl::RCSScale.y, &RecoilMin, &RecoilMax, "%.1f", ImGuiSliderFlags_None);
				ImGui::Checkbox(u8"掩体不瞄", &MenuConfig::VisibleCheck);
				ImGui::Text(u8"[INSERT] 隐藏菜单");
			}
			else if (tabb == 2) {
				//radar
				ImGui::SeparatorText(u8"杂项");
				ImGui::Checkbox(u8"雷达", &MenuConfig::ShowRadar);
				ImGui::Combo(u8"雷达类型", &MenuConfig::RadarType, u8"圆圈\0箭头\0带箭头的圆圈");

				ImGui::Checkbox(u8"十字线", &MenuConfig::ShowRadarCrossLine);
				ImGui::SameLine();
				ImGui::ColorEdit4("##CrossLineColor", reinterpret_cast<float*>(&MenuConfig::RadarCrossLineColor), ImGuiColorEditFlags_NoInputs);

				float ProportionMin = 500.f, ProportionMax = 5000.f;
				float RadarRangeMin = 100.f, RadarRangeMax = 500.f;
				float RadarBgAlphaMin = 0.0f, RadarBgAlphaMax = 1.0f;
				float RadarPointSizeProportionMin = 0.8f, RadarPointSizeProportionMax = 2.f;
				Gui.SliderScalarEx1(u8"点大小", ImGuiDataType_Float, &MenuConfig::RadarPointSizeProportion, &RadarPointSizeProportionMin, &RadarPointSizeProportionMax, "%.1f", ImGuiSliderFlags_None);
				Gui.SliderScalarEx1(u8"比例", ImGuiDataType_Float, &MenuConfig::Proportion, &ProportionMin, &ProportionMax, "%.1f", ImGuiSliderFlags_None);
				Gui.SliderScalarEx1(u8"尺寸", ImGuiDataType_Float, &MenuConfig::RadarRange, &RadarRangeMin, &RadarRangeMax, "%.1f", ImGuiSliderFlags_None);
				Gui.SliderScalarEx1(u8"背景透明度", ImGuiDataType_Float, &MenuConfig::RadarBgAlpha, &RadarBgAlphaMin, &RadarBgAlphaMax, "%.3f", ImGuiSliderFlags_None);

				//float WatermarkAlphaMax = 1.0f;
				//float WatermarkAlphaMin = 0.0f;
				ImGui::Checkbox(u8"水印", &MenuConfig::WaterMark);
				ImGui::SameLine();
				if (ImGui::Button(u8"设置"))
					ImGui::OpenPopup("##...");
				if (ImGui::BeginPopup("##..."))
				{
					ImGui::TextUnformatted(u8"设置");
					ImGui::Checkbox(u8"插件名称", &MenuConfig::watermarkcheat);
					ImGui::Checkbox(u8"帧数显示", &MenuConfig::watermarkfps);
					ImGui::Checkbox(u8"时间显示", &MenuConfig::watermarktime);
					ImGui::Checkbox(u8"用户名称", &MenuConfig::watermarkuser);
				...
				//triggerbot
				ImGui::SeparatorText(u8"自动开火");

				ImGui::Checkbox(u8"自动开火", &MenuConfig::TriggerBot);
				ImGui::SameLine();
				HotKey(&TriggerBot::HotKey, ImVec2(95, 28));

				if (ImGui::Combo(u8"开火键", &MenuConfig::TriggerHotKey, "MENU\0RBUTTON\0XBUTTON1\0XBUTTON2\0CAPITAL\0SHIFT\0CONTROL"))
				{
...
void Cheats::RadarSetting(Base_Radar& Radar)
{
	...
	// Radar window
	ImGui::Begin(u8"雷达", 0, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar);
